...
const updateEmployeesController = async (req, res) => {
    ...
    try {
        ...
        await pool.query(
            'UPDATE employees SET name = $1, email = $2, designation = $3 WHERE empid = $4',
            [updatedName, updatedEmail, updatedDesignation, empid]
        );
...
const deleteEmployeesController = async (req, res) => {
    ...
    try {
        const checkEmp = await pool.query("SELECT * FROM employees WHERE empid = $1", [empIdNum]);
        if (checkEmp.rows.length === 0) {
            return res.status(400).json({ message: "Employee not found" });
        }
